...
ADMIN имеет полный доступ к кандидатам, пользователям и черновикам оценок. RECRUITER видит всех кандидатов и управляет кандидатами и пользователями, кроме администраторов. Только ADMIN может назначать роль ADMIN и изменять учётные записи администраторов. MANAGER видит и оценивает назначенных ему кандидатов; при создании кандидата автоматически добавляется в hiring managers. Доступ проверяется SQL scope и Laravel Policies. Завершённые оценки неизменяемы для всех ролей; авторство сохраняется и при работе администратора с чужим черновиком.

Роли меняются в разделе «Пользователи» → «Редактировать». Связи с кандидатами и история оценок при смене роли сохраняются. Существующие участники остаются в карточках, но новые назначения проверяются по текущей роли и активности. Кандидаты с историей и связанные пользователи не удаляются — используйте статус «Отклонён» или отключение учётной записи. Последний активный ADMIN защищён от удаления, отключения и понижения роли. До назначения первого ADMIN аналогичная защита действует для последнего активного RECRUITER.

Первого администратора можно назначить существующей активной учётной записи, не меняя её пароль:

```sh
docker compose exec app php artisan hrsquare:make-admin LOGIN
```

### Пароли и первый вход

При создании пользователя через интерфейс задаётся временный пароль `QazWsx123456`. API также использует его, если пароль не указан. Все новые пользователи и пользователи после административного сброса пароля должны сменить его: до этого сервер разрешает только `/api/me`, `/api/logout` и `PUT /api/me/password`. Интерфейс направляет на «Задайте личный пароль».

После первого входа смена доступна в меню «Пароль и безопасность»: текущий пароль, новый пароль и подтверждение. Новый пароль — от 8 до 255 символов, отличается от текущего и общего временного пароля. Для обязательной первой смены текущий пароль повторно не требуется. Пароли хранятся только в хешированном виде и не возвращаются API. Смена пароля ограничена 5 запросами в минуту на пользователя, защищена CSRF. При смене/сбросе увеличивается версия авторизации: другие сеансы завершаются при следующем запросе; собственный сеанс после смены сохраняется.

Миграция `2026_09_17_000001_add_admin_and_password_change` добавляет роль и признаки безопасности. Существующие пароли и роли автоматически не меняются, пользователей не назначают администраторами без явного выбора. Для уже существующего пользователя обязательная смена включается при административном сбросе пароля. Кандидат остаётся карточкой HR, отдельная учётная запись кандидату не создаётся.
